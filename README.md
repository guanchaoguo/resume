...
### 深圳三四时代公司 （ 2017年3月 ~ 2018年4月 ）
    ...
    项目名称: 乐博游戏后方总平台
        ...
        项目描述:
           ...
           商交收，游戏配置和客服人员管理等
        承担的职责:
            1 独立负责基于golang的高性能游戏登录网关API
            2 实现二进制协议的与游戏端C++实时交互
            3 实现基于RabiitMq的消息队列与游戏端异步通讯
            4 实现mongo数据库的海量日志分片和查询
            5 实现600W用户表的水分拆分

    核心技术栈:
        1. mysql数据存贮  mongo存贮游戏日志 Redis存放玩家登陆信息
        2. Nosql redis mango实现数据缓存和日志处理
        3. 实时持久异步的强制性一致性的数据处理
        4. TCP二进制协议快速轻量的数据格式
        5. 利用基于角色的权限控制细分权限
